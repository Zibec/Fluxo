...
export function BudgetDetailCard({ budget, onEdit }: BudgetDetailCardProps) {
	...
	useEffect(() => {
		const fetchCategoriaNome = async () => {
			await categoriasService
				.getCategoriaById(budget.categoriaId)
				.then((data) => {
	...
	useEffect(() => {
		const fetchTransacoes = async () => {
			const transacoes: createTransacaoFormData[] =
				await transacaoService.getTransacoesByUser();
			setTransacoes(
				transacoes.filter((t) => t.categoriaId === budget.categoriaId),
			);
		};
		fetchTransacoes();
	}, []);
					...
					<div className="space-y-2">
						<div className="flex items-center justify-between text-sm">
							...
							<span className="font-medium">
								{getCurrencySymbol()} {spent.toFixed(2)} / {getCurrencySymbol()}{" "}
